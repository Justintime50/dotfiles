...
if [[ ! -d "$HOME"/dotfiles ]] ; then
    git clone https://github.com/Justintime50/dotfiles.git "$HOME"/dotfiles > /dev/null || echo "Error cloning Dotfiles"; exit 1
else
    git -C "$HOME"/dotfiles pull > /dev/null || echo "Error pulling Dotfiles"; exit 1
fi
