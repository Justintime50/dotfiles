...
AWS_DEVVM_EL6_INSTANCE_NAME="oregon1"
...
# SSH
alias ssh-oregon1='ssh -A $AWS_DEVVM_USER@$AWS_DEVVM_EL6_INSTANCE_NAME.$EP_USERNAME.$AWS_DEVVM_HOST'
alias ssh-oregon1-jump='ssh -A -J `whoami`@$AWS_DEVVM_BASTION_HOST $AWS_DEVVM_USER@$AWS_DEVVM_EL6_INSTANCE_NAME.`whoami`.devvm.easypo.net'
...
alias scp-upload-oregon1="do_scp_upload_oregon1"
do_scp_upload_oregon1() {
    if [ -z "$1" ]; then
        echo "You must provide a filename to use SCP."
    else
        scp "$@" "$AWS_DEVVM_USER"@"$AWS_DEVVM_EL6_INSTANCE_NAME"."$EP_USERNAME"."$AWS_DEVVM_HOST":"$AWS_UPLOADS_DIR" || echo "Couldn't copy file."
    fi
}

alias scp-download-oregon1="do_scp_download_oregon1"
do_scp_download_oregon1() {
    if [ -z "$1" ]; then
        echo "You must provide a filename to use SCP."
    else
        scp "$AWS_DEVVM_USER"@"$AWS_DEVVM_EL6_INSTANCE_NAME"."$EP_USERNAME"."$AWS_DEVVM_HOST":"$1" "$HOME"/Downloads/ || echo "Couldn't copy file."
    fi
}
