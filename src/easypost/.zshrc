...
AWS_DEVVM_OREGON2_INSTANCE_NAME="oregon2"
...
# SSH
alias ssh-oregon2='ssh -A $AWS_DEVVM_USER@$AWS_DEVVM_OREGON2_INSTANCE_NAME.$EP_USERNAME.$AWS_DEVVM_HOST'
alias ssh-oregon2-jump='ssh -A -J `whoami`@$AWS_DEVVM_BASTION_HOST $AWS_DEVVM_USER@$AWS_DEVVM_OREGON2_INSTANCE_NAME.`whoami`.devvm.easypo.net'
...
do_scp_upload_oregon2() {
    if [ -z "$1" ]; then
        ...
        scp "$@" "$AWS_DEVVM_USER"@"$AWS_DEVVM_OREGON2_INSTANCE_NAME"."$EP_USERNAME"."$AWS_DEVVM_HOST":"$AWS_UPLOADS_DIR" || echo "Couldn't copy file."
    fi
}
...
do_scp_download_oregon2() {
    if [ -z "$1" ]; then
        ...
        scp "$AWS_DEVVM_USER"@"$AWS_DEVVM_OREGON2_INSTANCE_NAME"."$EP_USERNAME"."$AWS_DEVVM_HOST":"$1" "$HOME"/Downloads/ || echo "Couldn't copy file."
    fi
}
